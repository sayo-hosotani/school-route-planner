...
| 通学路の保存 | 現在の経路をメモリに保存（セッション中のみ有効） |
...
### メモリ上での保存
- 経路データはモジュールレベル変数（`storedRoutes`）でメモリ上に保持
- ブラウザリロードやタブを閉じるとデータはクリアされる
- セキュリティ上の理由からlocalStorageは使用しない（XSS攻撃によるデータ漏洩リスクの排除）
- 永続化が必要な場合はJSONエクスポート/インポート機能を利用する
