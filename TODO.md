...
### 1.1. App.tsx の改善
現在415行と肥大化しており、責務が多すぎる。

- [ ] カスタムフックへの分離
  - `usePoints`: ポイントの追加・編集・削除・移動・順序変更ロジック
  - `useRouteGeneration`: Valhalla API呼び出し、経路生成ロジック
  - `useMessage`: メッセージ表示・自動消去ロジック（重複パターンの解消）
  - `useModal`: モーダル開閉状態の管理
- [ ] 重複コードの解消
  - メッセージ表示パターン（`setMessage` + `setTimeout`）が10箇所以上で重複
  - 経路更新パターン（`setPoints` + `updateRouteLine`）が5箇所で重複
- [ ] デバッグコードの削除
  - 75-87行目にconsole.logが残存
- [ ] 定数の外部化
  - メッセージタイムアウト（3000ms）
  - 初期座標（[35.628, 139.596]）
  - 初期ズームレベル（14）
