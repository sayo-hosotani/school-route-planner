...
- **機能**:
  - 地図の閲覧・移動・ズーム
  - 既存のポイント・経路・コメントの表示
  - ポイント上にコメントを常時表示（吹き出しTooltip、読み取り専用）
  - **サイドバーでのポイント操作**（実装済み）:
  ...
  - **経路の保存**（実装済み）:
    - 現在表示中の経路をDBに保存
    - 保存時に経路名を入力するモーダルを表示
    - 経路名は必須入力（デフォルト: `経路 YYYY/MM/DD HH:mm:ss`）
    - 保存ボタン押下でモーダル表示 → 経路名入力 → 保存
  - **保存済み経路の管理**（実装済み）:
    - サイドバーに保存済み経路の一覧を表示
    - 一覧には経路名と作成日時を表示（`YYYY/MM/DD HH:mm`形式）
    - 経路をクリックして地図に読み込み
    - 経路の削除ボタン（ゴミ箱アイコン、確認ダイアログ付き）
    - 新しい経路を上に表示（created_at降順）
...
│                                     │
│ 保存済み経路 (3):                   │
│ ・通勤経路 2026/01/03 12:30 [🗑]   │
│ ・お出かけ 2026/01/02 15:45 [🗑]   │
│ ・テスト経路 2026/01/02 10:00 [🗑] │
│                                     │
│ 【編集モード】                      │
│ ・地図をクリックしてポイント追加     │
│ ・ポイントをクリックして編集        │
...
- `components/`: 再利用可能なReactコンポーネント
  - **実装済みコンポーネント:**
    - `Sidebar.tsx`: モード切り替えと機能ボタン、ポイント一覧表示（並び替えボタン付き）、保存済み経路一覧表示
    - `PointEditModal.tsx`: ポイント編集モーダル（種別・コメント編集）
    - `RouteNameModal.tsx`: 経路名入力モーダル（経路保存時に表示）
...
- `api/`: バックエンドAPIとの通信処理
  - `route-api.ts`: 経路保存・読み込み・生成API呼び出し
    ...
    - `saveRoute()`: 経路データをデータベースに保存（経路名パラメータ付き）
    - `getAllRoutes()`: 保存済み経路一覧を取得
    - `loadRouteById()`: 特定の経路をIDで読み込み
    - `deleteRoute()`: 経路を削除
